...
#define BLOCK_COUNT  8 // this is the last block in the list, not the ammount of possible blocks
#define SEA_LEVEL 999

lpa::Color blockColor(int type) {
    switch (type) {
        case BLOCK_GRASS: return lpa::Color(106, 255, 62);
        case BLOCK_DIRT:  return lpa::Color(134, 96,  67);
        case BLOCK_STONE: return lpa::Color(128, 128, 128);
        case BLOCK_PLANKS: return lpa::Color(196, 180, 163);
        case BLOCK_LOGS: return lpa::Color(50, 25, 0);
        case BLOCK_SAND: return lpa::Color(242, 222, 177);
        case BLOCK_LEAVES: return lpa::color::bright::MAGENTA;
        default:          return lpa::color::WHITE;
    }
}

uint8_t selected = BLOCK_GRASS;
            ...
            for (int y = 0; y < CY; y++) {
                ...
                if (y < height) {
                    // sky / water (above ground)
                        chunk[wx][wz][x][z][y] = BLOCK_AIR;
                }

                else if (y == height) {
                    // surface
                    if (biome == BIOME_DESERT) {
                        chunk[wx][wz][x][z][y] = BLOCK_SAND;
                    }
                    else if (biome == BIOME_MOUNTAINS) {
                        if (height > SEA_LEVEL + 10)
                            chunk[wx][wz][x][z][y] = BLOCK_STONE;
                        else
                            chunk[wx][wz][x][z][y] = BLOCK_GRASS;
                    }
                    else {
...
int main() {
    ...
    lpa::Window win("Mycraps Alpha 1.0", SDL_WINDOWPOS_CENTERED, SDL_WINDOWPOS_CENTERED, 800, 600);
    ...
    while (appRunning) {
        ...
        while (menuRunning) {
            ...
            if (menuState == 0) {
                ...
                win.drawText("MyCraps", 220, 180, font, lpa::color::WHITE);
        ...
        while (running) {
            ...
            if (selected == BLOCK_GRASS) blockstr += "Grass";
            if (selected == BLOCK_DIRT) blockstr += "Dirt";
            if (selected == BLOCK_STONE) blockstr += "Stone";
            if (selected == BLOCK_PLANKS) blockstr += "Planks";
            if (selected == BLOCK_LOGS) blockstr += "Logs";
            if (selected == BLOCK_SAND) blockstr += "Sand";
            if (selected == BLOCK_LEAVES) blockstr += "Leaves";
